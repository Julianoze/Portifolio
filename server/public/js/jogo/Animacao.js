class Animacao {
	constructor() {
		...
		this._slider = document.getElementById("slick-carousel")
		...
		this._explosaoImage = new Image()
		this._explosao.src = '/sound/explosao.mp3'
		this._explosaoImage.src = '/style/img/explosao.png'
				...
				if ((data.getTime() - this._time[i].getTime()) > 50) {
					for (let j in this._excluidos) {
						...
						if (this._excluidos[j]._i == 0 || this._excluidos[j]._i == 70 || this._excluidos[j]._i == 140 || this._excluidos[j]._i == 210 || this._excluidos[j]._i == 280) {
							this._context.drawImage(this._explosaoImage, this._excluidos[j]._i, 0, 70, 70, this._excluidos[j]._x, this._excluidos[j]._y, (this._explosaoImage.width / 5) * 2, (this._explosaoImage.height) * 2)
			...
			for (let j in this._sprite) {
				...
				//Se o sprite I for do tipo tiro e o Sprite J do tipo professor verifica colisão
					...
					if ((this._sprite[i]._x > this._sprite[j]._x) && (this._sprite[i]._x < this._sprite[j]._xFinal) && (this._sprite[i]._y <= this._sprite[j]._yFinal)) {
						...
						this._reveal[0].style = "display: block"
	...
	conteudo () {
		this._revealDiv.innerHTML = `<center>
            <h1>Título</h1>
        </center>
        <div class="row">
            <div class="large-3 medium-3 imagem column"></div>
            <div class="large-9 medium-3 column end p-background">
                <p class="p-modal">Lorem ipsum dolor sit amet consectetur adipisicing elit. Consequuntur perspiciatis aliquam reprehenderit,
                    nisi fuga magni! Perferendis earum assumenda eaque alias, maiores dolores quos, impedit consequatur pariatur
                    quo explicabo numquam reiciendis! Lorem ipsum, dolor sit amet consectetur adipisicing elit. Numquam doloribus
                    ducimus asperiores voluptas a amet excepturi impedit aperiam nesciunt dicta. Cum quos excepturi facere
                    dolorem fugit vitae temporibus laudantium quibusdam sed eos fuga alias minima quis in quisquam at! Lorem
                    ipsum dolor sit, amet consectetur adipisicing elit. Rem aliquid ducimus, accusamus numquam vero omnis
                    nisi quo vitae at tenetur nulla, voluptatibus optio reprehenderit culpa quam cumque iste iure architecto.</p>
            </div>
        </div>
        <div class="row">
            <div class="large-12 p-background">
                <p class="p-modal">Lorem ipsum dolor sit amet, consectetur adipisicing elit. In, sequi provident inventore laborum ipsa harum?
                    Laboriosam aperiam suscipit nesciunt accusantium impedit ipsa, eum odio, similique aliquam optio eius.
                    Omnis, laboriosam! Lorem ipsum dolor sit amet consectetur adipisicing elit. Eius asperiores similique
                    itaque maxime? Reiciendis, quisquam veritatis ratione quo unde odio veniam fugiat incidunt obcaecati
                    sunt hic, dolore illo reprehenderit neque?
                </p>
            </div>
        </div>
        <center>
            <div class="slick large-12">
            </div>
		</center>`
